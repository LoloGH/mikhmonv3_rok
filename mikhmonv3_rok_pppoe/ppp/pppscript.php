...
if (!function_exists('pppOnUpScript')) {
  function pppOnUpScript($name, $expmode, $price, $sprice, $validity, $lockmode)
  {
    ...
    $stamp = '{:local comment [ /ppp secret get [/ppp secret find where name="$user"] comment];'
      ...
      . ' :local note ""; :if ($ucode = "pp") do={:set note [:pic $comment 3 [:len $comment]]};'
      ...
      . ' :if ($getxp = 15) do={ :local d [:pic $exp 0 6]; :local t [:pic $exp 7 16]; :local s ("/"); :local exp ("$d$s$year $t"); /ppp secret set comment="$exp $note" [find where name="$user"];};'
