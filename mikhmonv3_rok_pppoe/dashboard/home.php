...
// get & counting pppoe clients
  include_once('./ppp/pppmisc.php');
  $countpppactive = $API->comm("/ppp/active/print", array("count-only" => ""));
  $getpppsecret = $API->comm("/ppp/secret/print");
  $countpppsecret = count($getpppsecret);
  $countpppexpired = 0;
  for ($i = 0; $i < $countpppsecret; $i++) {
    if (pppStatus($getpppsecret[$i]['comment'], $getpppsecret[$i]['disabled']) == "expired") {
      $countpppexpired++;
    }
  }
  if ($countpppsecret < 2) {
              ...
              <div class="card-header"><h3><i class="fa fa-plug"></i> PPPoE</h3></div>
                <div class="card-body">
                  <div class="row">
                    ...
                    <div class="col-3 col-box-6">
                      <div class="box bg-green bmh-75">
                        <a onclick="cancelPage()" href="./?ppp=secrets&pprofile=all&session=<?= $session; ?>">
                          <h1><?= $countpppsecret; ?>
                              <span style="font-size: 15px;"><?= $sunit; ?></span>
                          </h1>
                          <div>
                            <i class="fa fa-users"></i> <?= $_pppoe_clients ?>
                          </div>
                        </a>
                      </div>
                    </div>
                    <div class="col-3 col-box-6">
                      <div class="box bg-red bmh-75">
                        <a onclick="cancelPage()" href="./?ppp=secrets&pprofile=all&pstatus=expired&session=<?= $session; ?>">
                          <h1><?= $countpppexpired; ?>
                              <span style="font-size: 15px;"><?= $_st_expired; ?></span>
                          </h1>
                          <div>
                            <i class="fa fa-hourglass-end"></i> <?= $_pppoe_clients ?>
                          </div>
                        </a>
                      </div>
                    </div>
                    <div class="col-3 col-box-6">
                      <div class="box bg-yellow bmh-75">
                        <a onclick="cancelPage()" href="./?ppp=addsecret&session=<?= $session; ?>">
                          ...
                          <div>
                            <i class="fa fa-user-plus"></i> <?= $_pppoe_clients ?>
                          </div>
                        </a>
                      </div>
                    </div>
